...
describe('System Console', () => {
    ...
    it('MM-T1100 Lock Divisionmate Name Display for all users', () => {
        // # Go to system admin page
        cy.visit('/admin_console/site_config/users_and_teams');

        // # Set Divisionmate Name Display dropdown to 'Show nickname if one exists, otherwise show first and last name' and set Lock Divisionmate Name Display to false
        ...
        // * Assert that there exists a description underneath the Divisionmate Name Display lock setting with the following text
        cy.findByTestId('TeamSettings.LockTeammateNameDisplayhelp-text').contains('When true, disables users\' ability to change settings under **Settings > Display > Divisionmate Name Display**.').should('be.visible');
        ...
        // # Click Edit button beside Divisionmate Name Display
        ...
        // * Assert the description under the Divisionmate Name Display title has changed to Show first and last name
        cy.get('#name_formatDesc').contains('Show first and last name').should('be.visible');

        // # Go back to System Admin console page
        cy.visit('/admin_console/site_config/users_and_teams');

        // # Set Divisionmate Name Display dropdown to 'Show by username' and set Lock Divisionmate Name Display to true
        ...
        // * Assert the description under the Divisionmate Name Display title has changed to Show username
        cy.get('#name_formatDesc').contains('Show username').should('be.visible');

        // # Click edit button for Divisionmate Name Display
        cy.get('#name_formatEdit').click();
